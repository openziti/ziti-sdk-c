...
void save_ziti_socket(const void *arg, future_t *f, uv_loop_t *l) {
    int fds[2];
    ...
    ziti_socket_t *zs = calloc(1, sizeof(ziti_socket_t));
    zs->fd = fds[0];
    zs->ziti_fd = fds[1];
    model_map_setl(&ziti_sockets, (long) zs->fd, zs);
    complete_future(f, (void *) zs->fd);
}

int Ziti_socket() {
    future_t *f = schedule_on_loop(save_ziti_socket, NULL, true);
    int rc = await_future(f);

    rc = rc == 0 ? (int) f->result : rc;
    destroy_future(f);
    return rc;
}
...
static void on_ziti_connect(ziti_connection conn, int status) {
    ziti_socket_t *zs = ziti_conn_data(conn);
    if (status == ZITI_OK) {
        ziti_conn_bridge_fds(conn, zs->ziti_fd, zs->ziti_fd, on_bridge_close, zs);
...
int Ziti_connect(int socket, ziti_context ztx, const char *service) {
    ...
    future_t *f = schedule_on_loop(do_ziti_connect, &req, true);
    int err = await_future(f);
    destroy_future(f);
    return err;
}

...
int parse_jwt_content(struct enroll_cfg_s *ecfg, ziti_enrollment_jwt_header *zejh, ziti_enrollment_jwt *zej) {
    ...
    ZITI_LOG(DEBUG, "ecfg->jwt_signing_input is: %s", ecfg->jwt_signing_input);
...
int load_jwt_file(const char *filename, struct enroll_cfg_s *ecfg, ziti_enrollment_jwt_header *jh, ziti_enrollment_jwt *zej) {
    ...
    return parse_jwt_content(ecfg, jh, zej);
}

int load_jwt(const char *filename, struct enroll_cfg_s *ecfg, ziti_enrollment_jwt_header *jh, ziti_enrollment_jwt *jwt) {

    ZITI_LOG(DEBUG, "filename is: %s", filename);

    if (filename != NULL) {
        return load_jwt_file(filename, ecfg, jh, jwt);
    }

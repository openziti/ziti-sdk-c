...
static void ziti_pr_send_bulk(ziti_context ztx) {
    ...
    if (model_list_size(&send_prs) > 0) {
        ...
        if (send_posture_ha(ztx, &send_prs) == ZITI_NOT_SUPPORTED) {
            ZTX_LOG(INFO, "ERs do not support receiving posture responses");
...
static Ziti__EdgeClient__Pb__PostureResponses *create_posture_resp(ziti_context z, model_list *send_prs) {
    ...
    MODEL_LIST_FOREACH(info, *send_prs) {
        ...
        case ziti_posture_query_type_PC_OS: {
            ...
            Ziti__EdgeClient__Pb__PostureResponse__OperatingSystem *os = calloc(1, sizeof(Ziti__EdgeClient__Pb__PostureResponse__OperatingSystem));
        ...
        case ziti_posture_query_type_PC_Process: {
            ...
            Ziti__EdgeClient__Pb__PostureResponse__Process *proc = calloc(1, sizeof(Ziti__EdgeClient__Pb__PostureResponse__Process));
        ...
        case ziti_posture_query_type_PC_MAC: {
            ...
            Ziti__EdgeClient__Pb__PostureResponse__Macs *mac = calloc(1, sizeof(Ziti__EdgeClient__Pb__PostureResponse__Macs));
    ...
    Ziti__EdgeClient__Pb__PostureResponses *resp = NULL;
...
static int send_posture_ha(ziti_context ztx, model_list *send_prs) {
    ...
    MODEL_MAP_FOREACH(n, ch, &ztx->channels) {
        if (ziti_channel_update_posture(ch, buffer.data, buffer.len) == ZITI_NOT_SUPPORTED) {
            err = ZITI_NOT_SUPPORTED;
        }
    }
...
void ziti_send_posture_er(ziti_context ztx, ziti_channel_t *ch) {
    if (!zch_can_accept_posture(ch)) {
        ZTX_LOG(DEBUG, "channel %s cannot accept posture responses", zch_get_name(ch));
        return;
    }

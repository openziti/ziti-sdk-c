...
ztx_wrap_t *zl_find_wrap(ziti_handle_t handle) {
    ...
    ztx_wrap_t *w;
    const char *id;
    MODEL_MAP_FOREACH(id, w, &ziti_contexts) {
        if (w->ztx->id == handle) {
            return w;
        }
    }
...
static void enroll_url_bootstrap_cb(const ziti_config *cfg, int status, const char *err, void *ctx) {
    ...
    if (status != ZITI_OK) {
        ZITI_LOG(ERROR, "failed to bootstrap from %s: %s", req->url, err ? err : ziti_errorstr(status));
        fail_future(req->enroll_f, status);
        return;
    }
...
static void do_totp_login(void *arg, future_t *f, uv_loop_t *l) {
    ...
    ztx_wrap_t *wrap = zl_find_wrap(ztx);
    if (wrap == NULL) {
        fail_future(f, EINVAL);
        return;
    }
...
static void wait_for_auth_cb(void *arg, future_t *f, uv_loop_t *l) {
    ...
    ztx_wrap_t *wrap = zl_find_wrap(h);
    if (wrap == NULL) {
        fail_future(f, EINVAL);
        return;
    }

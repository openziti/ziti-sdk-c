# local overlay: upstream always builds botan's CLI tool alongside the library, but
# ziti-sdk-c only ever links botan::botan-static (see tests/integ/CMakeLists.txt) -- the
# CLI's sandbox.cpp references the legacy macOS sandbox_init() constant
# kSBXProfileNoWriteExceptTemporary, which newer macOS SDKs have dropped from their headers,
# breaking the build. Dropping ",cli" from --build-targets below skips building it (and the
# now-dead vcpkg_copy_tools() calls that would otherwise try to package it), which is safe
# here since nothing in this repo uses the botan CLI binary on any platform.
vcpkg_from_github(
...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
    ...
    if(VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
        vcpkg_list(APPEND configure_arguments --build-targets=shared)
    else()
        vcpkg_list(APPEND configure_arguments --build-targets=static)
    endif()
